...
class RQBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.pre_quant = nn.Conv1d(
            ...
            out_channels=self.embed_dim * self.latent_shape[-1],
            kernel_size=1,
            # groups=self.latent_shape[-1]
        )
        self.post_quant = nn.Conv1d(
            in_channels=self.embed_dim * self.latent_shape[-1],
    ...
    def to_code_shape(self, x):
        ...
        # (B, C, L) -> (B, embed_dim * C, L)
        x = self.pre_quant(x)
        # (B, embed_dim * C, L) -> (B, L, embed_dim * C)
        x = x.permute(0, 2, 1).contiguous()
        # (B, L, embed_dim * C) -> (B * L * C, embed_dim)
        x_flatten = x.reshape(-1, self.embed_dim)

        return x_flatten

    def to_latent_shape(self, x, B, L):
        # x 是量化器的输出, 形状为 (B * L * C, embed_dim)
        C = self.latent_shape[-1]
        assert x.shape[0] == B * L * C, f"Input sequence length {x.shape[0]} does not match B*L*C ({B*L*C})"
        # (B * L * C, embed_dim) -> (B, L, embed_dim * C)
        x = x.view(B, L, self.embed_dim * C)
        # (B, L, embed_dim * C) -> (B, embed_dim * C, L)
        x = x.permute(0, 2, 1).contiguous()
        # (B, embed_dim * C, L) -> (B, C, L)
        x = self.post_quant(x)
        # (B, C, L) -> (B, L, C)
        x = x.permute(0, 2, 1).contiguous()
