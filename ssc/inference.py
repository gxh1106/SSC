...
def main():
    ...
    fa_system = FA_IM_Channel(K=K, M=M, N=N, Nr=Nr, num_H=num_H, W=W, L_paths=L_paths, device=device, codebook_size=codebook_size, CSI_error_TX=CSI_error_TX, CSI_error_RX=CSI_error_RX)

    M_fas_simo = 256  # SIMO FAS 的星座大小
    # M_fas_simo = 16
    # fa_system = FA_SISO_Channel(M=M_fas_simo, N=N, Nr=Nr, num_H=num_H, W=W, L_paths=L_paths, device=device, codebook_size=codebook_size)
    ...
    for path in tqdm(image_paths, desc="Processing Images"):
        ...
        for snr in snr_range:
            ...
            for idx_H in range(num_H):
                with torch.no_grad():
                    output = model.forward_faim(img_gt, given_SNR=snr, channel=fa_system, idx_H=idx_H)[0]
